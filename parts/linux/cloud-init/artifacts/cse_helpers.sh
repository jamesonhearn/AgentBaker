...
_retrycmd_internal() {
    ...
    for i in $(seq 1 "$retries"); do
        ...
        if ! check_cse_timeout "$shouldLog"; then
            ...
            RETRYCMD_INTERNAL_LAST_EXIT_STATUS=$exitStatus
            return 2
        fi
...
describe_curl_exit_code() {
    ...
    6)
        ...
        ;;
    7)
        echo "TCP connection failed while contacting the required endpoint"
        ;;
    28)
        echo "The connection attempt timed out while contacting the required endpoint"
        ;;
    35)
